...
import { linuxDoClassicTopicPath } from "../domain/linuxdo/urls";
...
export function normalizeTopicList(
  ...
  return {
    ...
    topics: (response.topic_list?.topics ?? [])
      .filter((topic): topic is DiscourseTopic & { id: number } =>
        Number.isFinite(topic.id)
      )
      .map((topic) => normalizeTopic(topic, users, categories)),
...
export function normalizeTopic(
  ...
  const category = topic.category_id
    ? categories.get(topic.category_id)
    : undefined;
  ...
  return {
    ...
    posters: normalizePosters(topic, users)
  };
}

export function cleanExcerpt(value: unknown): string {
  return stripEmojiShortcodes(cleanHtml(value))
    .replace(/\s+/g, " ")
    .trim()
    .slice(0, 240);
}
  ...
  return (topic.posters ?? [])
    .map((poster) => {
      ...
      return {
        ...
        avatarUrl: avatarUrl(user?.avatar_template, username),
...
function pickThumbnail(topic: DiscourseTopic): string {
  const thumbnail =
    topic.thumbnails?.find((item) => item.url && (item.max_width ?? 0) >= 200) ||
    topic.thumbnails?.find((item) => item.url) ||
    undefined;
  return absoluteUrl(thumbnail?.url || topic.image_url || "");
}

function avatarUrl(template: unknown, username: string): string {
  const avatarTemplate = textValue(template);
  if (!avatarTemplate) {
    return `https://ui-avatars.com/api/?background=0f172a&color=fff&name=${encodeURIComponent(
      username
    )}`;
  }
  return absoluteUrl(avatarTemplate.replace("{size}", "64"));
}
