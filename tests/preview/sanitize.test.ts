...
import { sanitizeCookedHtml } from "../../src/preview/sanitize";
...
describe("sanitizeCookedHtml", () => {
  ...
  it("normalizes oneboxes into compact reader link cards", () => {
    ...
    expect(onebox?.querySelector(".ldcv-reader-onebox__description")?.textContent).toBe(
      "Onebox summary"
    );
  ...
  it("marks reader images and removes duplicate attachment labels", () => {
    const html = sanitizeCookedHtml(`
      ...
      <p><a href="/uploads/default/original/1X/photo.png">image690×388 12.3 KB</a></p>
    `);
    const doc = parseHtml(html);
    ...
    expect(image?.getAttribute("data-reader-image-src")).toContain(
  ...
  it("normalizes collapsible details into reader details blocks", () => {
    ...
    expect(details?.querySelector(".ldcv-reader-details__body")?.textContent).toContain(
      "此文本将被隐藏"
    );
  });
